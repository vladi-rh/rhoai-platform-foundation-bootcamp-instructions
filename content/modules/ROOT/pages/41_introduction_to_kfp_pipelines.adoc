...
. Run the `09_secrets_cm_pipeline.py.yaml`. This pipeline shows how we can access secrets and config maps in out pipelines.
+
...
. Run the `10_mount_pvc_pipeline.py.yaml`. This pipeline shows how we can access pvcs in a pipeline.
