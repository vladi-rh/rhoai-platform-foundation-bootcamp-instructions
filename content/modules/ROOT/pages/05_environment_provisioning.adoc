...
. Navigate to demo.redhat.com and order the https://catalog.demo.redhat.com/catalog?item=babylon-catalog-prod%2Fsandboxes-gpte.ocp-wksp.prod[_**Red Hat OpenShift Container Platform Cluster(AWS)**_] (Note: this is a different link / catalog item from the one above)
